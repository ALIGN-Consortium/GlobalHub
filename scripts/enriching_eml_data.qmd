...
def best_rxnorm_name(record):
    ...
    Priority:
      1) RxNorm IN / PIN / MIN synonym
      2) LLM name
      3) Original name
    Returns None if nothing usable exists.
    """

    # 1️⃣ RxNorm ingredient names
    rxnorm_names = [
...
for r in master_records:
    rxcuis = [i["value"] for i in r.get("identifiers", []) if i.get("type") == "RxNorm"]

    fda_query_rows.append(
        {
            "eml_id": r.get("eml_id"),
            "scope": r.get("scope"),
            "fda_query_name": best_rxnorm_name(r),
            "rxcuis": rxcuis,
            "resolution_status": r.get("resolution", {}).get("status"),
        }
    )

fda_query_df = pd.DataFrame(fda_query_rows)
