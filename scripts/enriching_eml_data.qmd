...
Not removing duplicates at this point because of membership tests downstream.
```{python}
#| label: concatenate
# FULL dataset (no deduplication)
eml_full = pd.concat([who_eml_c, sa_eml_c, kenya_eml_c, senegal_eml_c])

# Deduplicated dataset for enrichment pipeline (unchanged behavior)
eml = eml_full.drop_duplicates(subset="name")

```

```{python}
#| label: save eml
eml_full.to_csv("/Users/jps89/Documents/GEMINI/ALIGN/GlobalHub/data/all_scope_eml_raw.csv")
```


```{python}
# | label: build-name-scope-map

from collections import defaultdict

name_scope_map = defaultdict(list)

for _, row in eml_full.iterrows():
    key = normalize_name(row["name"])

    name_scope_map[key].append(
        {
            "scope": row["scope"],
...
with open("atc_records.jsonl", "r", encoding="utf-8") as f:
    for line in f:
        atc_records.append(json.loads(line))
...
# commented to prevent rerunning
# for t in tests:
...
# commented to prevent re-running everytime - reading from memory later
...
# commented to prevent re-running
# llm_records = []
...
# with open("eml_llm_records.jsonl", "w", encoding="utf-8") as f:
...
with open("eml_llm_records.jsonl", "r", encoding="utf-8") as f:
    for line in f:
        llm_records.append(json.loads(line))

len(llm_records)

# Adding llm name back to the unresolved frame without re runinng the llm
llm_lookup = {
    rec["original_name"]: rec["input_name"]
    for rec in llm_records
}
unresolved["llm_name"] = unresolved["name"].map(llm_lookup)
```
...
def build_master_record(row):
    ...
    membership = name_scope_map.get(eml_key, [])
...
with open("eml_master.jsonl", "r", encoding="utf-8") as f:
    for line in f:
