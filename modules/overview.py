...
def overview_ui(id):
    ...
    def kpi_card(value, label, sub, icon):
        return ui.div(
            ...
            ui.div(ui.tags.i(class_=f"fa fa-{icon} fa-2x text-primary opacity-50")),
    ...
    return ui.div(
        ...
        ui.div(
            ui.div(
                ui.div(
                    ui.p("Track and assess health technologies for policy implementation", class_="mb-1 fs-5 text-white"),
                    ui.p("Innovations tracked", class_="mb-0 opacity-75 text-white"),
                    class_="col-md-8",
                ),
                ui.div(
                    ui.div(
                        ui.div(
                            ui.h4("47 of 60", class_="hero-highlight-text mb-1"),
                            ui.tags.small("Policy Maker Access", class_="hero-highlight-text opacity-75"),
                            ui.div(
                                ui.div(class_="progress-bar", style="width: 78%"),
                                class_="progress mt-2",
                                style="height: 6px;",
                            ),
                            class_="card-body text-center",
                        ),
                        class_="card bg-white bg-opacity-10 border-0",
                    ),
                    class_="col-md-4 text-end",
                ),
                class_="row align-items-center",
            ),
            class_="p-4 mb-4 rounded hero-section",
        ),
        # KPI cards
        ui.div(
            ui.div(ui.card(kpi_card("624", "Active Innovations", "+18% from last quarter", "wave-square")), class_="col-md-3"),
            ui.div(ui.card(kpi_card("156", "Implementation Ready", "+23% approved this quarter", "circle-check")), class_="col-md-3"),
            ui.div(ui.card(kpi_card("89", "Maternal Health Focus", "+12% new innovations", "baby")), class_="col-md-3"),
            ui.div(ui.card(kpi_card("8.7/10", "Impact Score", "+0.3 average improvement", "star")), class_="col-md-3"),
            class_="row mb-4 g-3",
        ),
        ...
        ui.div(
            ui.div(
                ...
                ui.input_select("disease_selector", None, choices=diseases, selected="Overall", width="220px"),
        ...
        ui.div(
            ui.div(
                ui.div(
                    ui.div("Forecast of products coming to market", class_="card-header"),
                    ui.div(output_widget("trend_chart", height="350px"), class_="card-body"),
                    class_="card",
                ),
                class_="col-md-7",
            ),
            ui.div(
                ui.div(
                    ...
                    ui.div(output_widget("pie_chart", height="350px"), class_="card-body"),
                    class_="card",
                ),
                class_="col-md-5",
            ),
        ...
        ui.div(
            ui.div(ui.value_box(
                "Total Budget Impact",
                "$196M",
                ui.tags.i(class_="fa fa-dollar-sign"),
                ui.tags.small("Savings projected over 5 years", class_="text-success"),
            ), class_="col-md-3"),
            ui.div(ui.value_box(
                "Implementation Readiness",
                "75%",
                ui.tags.i(class_="fa fa-gauge-simple"),
                ui.tags.small("Average across target countries", class_="text-success"),
            ), class_="col-md-3"),
            ui.div(ui.value_box(
                "Population Coverage",
                "412M",
                ui.tags.i(class_="fa fa-users"),
                ui.tags.small("People potentially covered", class_="text-success"),
            ), class_="col-md-3"),
            ui.div(ui.value_box(
                "Risk Level",
                "Medium",
                ui.tags.i(class_="fa fa-triangle-exclamation"),
                ui.tags.small("Overall implementation risk", class_="text-warning"),
            ), class_="col-md-3"),
            class_="row mt-3 g-3",
        ),
        # Analytics tabs
        ui.div(
            ui.div(
                ui.div(
                    ...
                    ui.input_select("impact_country", None, choices=["Overall", "Kenya", "Senegal", "South Africa"], selected="Overall", width="220px"),
            ...
            ui.div(
                ui.navset_tab(
                    ui.nav_panel("Impact Analysis",
                        ui.div(
                            ui.div(
                                ui.div(
                                    ui.div("Innovation Impact by Category", class_="card-header"),
                                    ui.div(
                                        ui.tags.div("Potential population impact and cost effectiveness", style="font-weight:400; font-size:13px; color:#6b7280;"),
                            ...
                            ui.div(
                                ui.div(
                                    ...
                                    ui.div(
                                        ui.tags.div("Innovations ready for policy implementation", style="font-weight:400; font-size:13px; color:#6b7280;"),
                    ...
                    ui.nav_panel("Budget Planning", ui.div(ui.output_ui("budget_chart"), class_="mt-3")),
                    ui.nav_panel("Implementation", ui.div(ui.output_ui("implementation_chart"), class_="mt-3")),
                    ui.nav_panel("Risk Assessment", ui.div(ui.output_ui("risk_chart"), class_="mt-3")),
                    ui.nav_panel("Country Readiness", ui.div(ui.output_ui("country_readiness_chart"), class_="mt-3")),
                    id="impact_tabs",
                ),
                class_="card-body",
            ),
        ...
        ui.div(
            ui.div(
                ui.div(
                    ...
                    ui.input_select("impact_country_table", None, choices=["Overall", "Kenya", "Senegal", "South Africa"], selected="Overall", width="220px"),
                    class_="d-flex align-items-center justify-content-between flex-wrap gap-2",
                ),
                class_="card-header",
            ),
...
def overview_server(id, input, output, session):
    ...
    default_innovation_id = horizon_df["Innovation"].iloc[0] if not horizon_df.empty else None
    ...
    def trend_chart():
        ...
        df_filtered = df_filtered[(df_filtered["market_year"] >= 2025) & (df_filtered["market_year"] <= 2050)]
        ...
        pipeline_raw = df_filtered.groupby(["market_year", "Category"]).size().unstack(fill_value=0)
        ...
        fig.update_layout(
            ...
            yaxis=dict(range=[0, None], fixedrange=True),  # Ensure Y-axis starts at 0 and prevent zooming below it
            annotations=[
                dict(
    ...
    @render.ui
    def impact_combo():
        country_data = impact_dat_all[impact_dat_all["country"] == input.impact_country()]
        ...
        bar = (
            ...
            .add_yaxis("Impact", rounded_impact, itemstyle_opts=opts.ItemStyleOpts(color="#00539B"))
            .extend_axis(
                yaxis=opts.AxisOpts(
        ...
        line = (
            Line()
            .add_xaxis(country_data["category"].tolist())
            .add_yaxis("Cost effectiveness", rounded_cost_eff, yaxis_index=1, is_smooth=True, itemstyle_opts=opts.ItemStyleOpts(color="#228B22"))
        )
        bar.overlap(line)
    ...
    @render.ui
    def readiness_bar():
        country_data = readiness_cat_all[readiness_cat_all["country"] == input.impact_country()]
        req(not country_data.empty)
        ...
        bar = (
            Bar(init_opts=opts.InitOpts(width="100%", height="300px"))
            .add_xaxis(country_data["category"].tolist())
            .add_yaxis("Total in pipeline", country_data["total"].tolist(), itemstyle_opts=opts.ItemStyleOpts(color="#BFBBBB"))
            .add_yaxis("Ready", country_data["ready"].tolist(), z=10, itemstyle_opts=opts.ItemStyleOpts(color="#228B22"))
            .set_global_opts(
    ...
    def budget_chart():
        ...
        bar = (
            ...
            .add_yaxis("Allocated (M)", budget_df["allocated"].round(0).astype(int).tolist())
    ...
    def implementation_chart():
        ...
        bar = (
            ...
            .add_yaxis("Planned", impl_df["planned"].round(0).astype(int).tolist(), stack="stack1")
            .add_yaxis("In Progress", impl_df["in_progress"].round(0).astype(int).tolist(), stack="stack1")
            .add_yaxis("Completed", impl_df["completed"].round(0).astype(int).tolist(), stack="stack1")
            .set_global_opts(
